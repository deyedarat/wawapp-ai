...
import 'package:core_shared/core_shared.dart' as core show Order;
...
class _StuckOrdersPanelState extends ConsumerState<StuckOrdersPanel> {
  ...
  Stream<List<core.Order>> _getStuckOrders() {
    ...
    return FirebaseFirestore.instance
        ...
        .limit(50)
        .snapshots()
        .map((snap) => snap.docs.map((doc) => core.Order.fromFirestoreWithId(doc.id, doc.data())).toList());
  }
  ...
  Future<String> _getDriverName(String? driverId) async {
    ...
    try {
      ...
      final data = doc.data();
      return data?['displayName'] as String? ?? data?['phoneNumber'] as String? ?? 'سائق #${driverId.substring(0, 6)}';
    } catch (_) {
      return 'خطأ';
    }
  }
  ...
  Future<String> _getCustomerName(String? ownerId) async {
    ...
    try {
      ...
      final data = doc.data();
      return data?['displayName'] as String? ?? data?['phoneNumber'] as String? ?? 'عميل #${ownerId.substring(0, 6)}';
    } catch (_) {
      return 'خطأ';
    }
  }
  ...
  Widget build(BuildContext context) {
    return AdminScaffold(
      ...
      actions: [IconButton(icon: const Icon(Icons.refresh), onPressed: () => setState(() {}), tooltip: 'تحديث')],
      child: StreamBuilder<List<core.Order>>(
                                  ...
                                  IconButton(
                                    ...
                                    onPressed: () {
                                      ScaffoldMessenger.of(
                                        context,
                                      ).showSnackBar(const SnackBar(content: Text('الاتصال بالسائق')));
                                    },
                                  ),
                                ],
                              ),
