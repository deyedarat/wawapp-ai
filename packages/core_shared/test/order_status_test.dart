...
void main() {
  group('OrderStatus', () {
    ...
    test('fromFirestore should handle legacy values', () {
      ...
      expect(OrderStatus.fromFirestore('cancelled'), OrderStatus.cancelledByDriver);
    });
    ...
    test('canTransitionTo should enforce valid transitions', () {
      expect(OrderStatus.requested.canTransitionTo(OrderStatus.assigning), true);
      ...
      expect(OrderStatus.requested.canTransitionTo(OrderStatus.completed), false);
    });

    test('createTransitionUpdate should include correct fields', () {
      final update = OrderStatus.accepted.createTransitionUpdate(driverId: 'driver123');
