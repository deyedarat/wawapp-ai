...
class DebugOrdersService {
  ...
  Stream<List<Map<String, dynamic>>> getAllOrdersDebug() {
    ...
    return _firestore
      ...
      for (final doc in snapshot.docs) {
        ...
        dev.log('[DEBUG] Order ${doc.id}: status=${data['status']}, pickup=${data['pickup']}, dropoff=${data['dropoff']}');
      }
  ...
  Stream<List<Map<String, dynamic>>> getOrdersByStatus(String status) {
    ...
    return _firestore
        ...
        .snapshots()
        .map((snapshot) {
      dev.log('[DEBUG] Orders with status "$status": ${snapshot.docs.length}');
  ...
  Stream<List<Map<String, dynamic>>> getNearbyOrdersUnlimited(Position driverPosition) {
    final targetStatus = OrderStatus.assigning.toFirestore();
    dev.log('[DEBUG] Looking for orders with status: "$targetStatus"');
    dev.log('[DEBUG] Driver position: lat=${driverPosition.latitude}, lng=${driverPosition.longitude}');
      ...
      for (final doc in snapshot.docs) {
        try {
          ...
          if (pickup['lat'] == null || pickup['lng'] == null) {
            dev.log('[DEBUG] ERROR: Order ${doc.id} pickup missing lat/lng: $pickup');
            continue;
          }
          ...
          dev.log('[DEBUG] Order ${doc.id}: distance=${distance.toStringAsFixed(1)}km, pickup=($pickupLat,$pickupLng)');
