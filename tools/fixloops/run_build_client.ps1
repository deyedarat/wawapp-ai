...
<#
.SYNOPSIS
    Build wawapp_client APK with logging.

.DESCRIPTION
    Runs flutter build apk for wawapp_client and captures all output to logs/build_client.log.
...
# Log file path
$LogFile = Join-Path $LogDir "build_client.log"
$Timestamp = Get-Date -Format "yyyy-MM-dd HH:mm:ss"

# Write header to log
"" | Out-File -FilePath $LogFile -Append
"=" * 80 | Out-File -FilePath $LogFile -Append
"[$Timestamp] Starting flutter build apk for wawapp_client..." | Out-File -FilePath $LogFile -Append
...
Write-Host "[$Timestamp] Building wawapp_client APK ($BuildMode)..." -ForegroundColor Cyan
...
$ApkPath = Join-Path $ClientPath "build\app\outputs\flutter-apk"
